...
const levelColors: Record<string, string> = {
  draft: 'bg-neutral-500/10 border-neutral-500/30 border-dashed',
...
<template>
  <div
    ...
    :class="[levelColors[data.level ?? 'draft'], statusClasses]"
  >
    <Handle
      v-if="data.level && data.level !== 'L0'"
      type="target"
    ...
    <div class="text-xs font-medium text-neutral-400 mb-0.5 shrink-0">
      {{ data.level ?? 'draft' }}{{ data.level ? ` • ${data.type}` : '' }}
    </div>
