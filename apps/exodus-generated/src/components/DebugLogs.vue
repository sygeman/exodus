...
<template>
  <div class="flex h-full flex-col bg-[var(--ui-bg)]">
    ...
    <UScrollArea class="min-h-0 flex-1" v-for="item in logs" :key="item.id">
      <div class="group border-b border-[var(--ui-border)] px-4 py-2 text-xs leading-relaxed hover:bg-[var(--ui-bg-elevated)]">
        <div class="group flex items-center gap-2">
          <span class="font-mono text-[var(--ui-text-muted)]">{{ formatTime(item.timestamp) }}</span>
          <UBadge color="neutral" variant="subtle" class="text-[10px] uppercase">{{ item.level }}</UBadge>
          <span class="rounded px-1.5 py-0.5 text-[10px] uppercase" :style="item.source === 'bun' ? 'background-color: rgba(59,130,246,0.1); color: #3b82f6' : 'background-color: rgba(16,185,129,0.1); color: #10b981'">{{ item.source }}</span>
          ...
          <UButton icon="i-lucide-copy" variant="ghost" size="xs" class="opacity-0 transition-opacity group-hover:opacity-100" @click="handleCopyLog(item)" />
        </div>
      </div>
    </UScrollArea>
    <div class="flex items-center justify-between border-t border-[var(--ui-border)] px-4 py-2">
      ...
      <div class="flex items-center gap-1">
        <UButton icon="i-lucide-chevrons-left" variant="ghost" size="xs" @click="handleFirstPage()" />
        <UButton icon="i-lucide-chevron-left" variant="ghost" size="xs" @click="handlePrevPage()" />
        <UButton icon="i-lucide-chevron-right" variant="ghost" size="xs" @click="handleNextPage()" />
        <UButton icon="i-lucide-chevrons-right" variant="ghost" size="xs" @click="handleLastPage()" />
      </div>
    </div>
  </div>
</template>
