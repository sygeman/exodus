...
export interface Project_flowsItem {
  id: string
  project_id: string
  name: string
  status: string | null
...
export function useProject_flows(options?: { filter?: Record<string, unknown>; sort?: string[] }) {
  const items = ref<Project_flowsItem[]>([])
  ...
  async function load(filter?: Record<string, unknown>) {
    ...
    try {
      const result = await edem.data.queryItems({
        collection_id: "project_flows",
        filter: filter ?? options?.filter ?? {},
        sort: options?.sort,
      })
      items.value = result.items as Project_flowsItem[]
    } finally {
      loading.value = false
    }
  }
  ...
  function subscribe() {
    unsubs.push(
      edem.data.itemCreated(async ({ event: item }) => {
        if (item.collection_id !== "project_flows") return
        if (items.value.some((i) => i.id === item.id)) return
        items.value.push(item as Project_flowsItem)
      }),
    )
    unsubs.push(
      edem.data.itemUpdated(async ({ event: item }) => {
        ...
        if (idx !== -1) items.value[idx] = item as Project_flowsItem
      }),
    )
    unsubs.push(
  ...
  async function create(data: Record<string, unknown>) {
    const result = await edem.data.createItem({ collection_id: "project_flows", data })
    await load()
    return result
  }
